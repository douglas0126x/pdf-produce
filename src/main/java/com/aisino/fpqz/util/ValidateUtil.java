...
public class ValidateUtil {
	...
	public static void validatePdf(JAR_FPQZ_KJ kj) throws CustomException {
		...
		if ((!Strings.isNullOrEmpty(kj.getGMF_YHZH()))
		...
		if (!Strings.isNullOrEmpty(kj.getGMF_MC())) {
			if (checkLength(kj.getGMF_MC(), 100))
				throw new CustomException(1015, new StringBuilder()
						...
						.append(")").toString());
		} else throw new CustomException(1016, "购买方名称必填");

		if ((!Strings.isNullOrEmpty(kj.getGMF_NSRSBH()))
		...
		if ((!Strings.isNullOrEmpty(kj.getXSF_DZDH()))
		...
		if (!Strings.isNullOrEmpty(kj.getXSF_MC())) {
		...
		if (!Strings.isNullOrEmpty(kj.getXSF_NSRSBH())) {
		...
		if (!Strings.isNullOrEmpty(kj.getFP_MW())) {
		...
		if (!Strings.isNullOrEmpty(kj.getJYM())) {
		...
		if (!Strings.isNullOrEmpty(kj.getFP_DM())) {
			if ((!checkLengthEquals(kj.getFP_DM(), 10))
				...
				throw new CustomException(1036, new StringBuilder()
						...
						.append(")").toString());
			if ((!isNumber(kj.getFP_DM())) || (kj.getFP_DM().indexOf(".") > -1))
		...
		if (!Strings.isNullOrEmpty(kj.getYFP_DM())) {
		...
		if (!Strings.isNullOrEmpty(kj.getYFP_HM())) {
		...
		if (!Strings.isNullOrEmpty(kj.getJQBH())) {
		...
		if (!Strings.isNullOrEmpty(kj.getJSHJ())) {
		...
		if (!Strings.isNullOrEmpty(kj.getHJJE())) {
		...
		if (!Strings.isNullOrEmpty(kj.getHJSE())) {
		...
		if ((!Strings.isNullOrEmpty(kj.getDKBZ()))
		...
		if (!Strings.isNullOrEmpty(kj.getSGBZ())) {
		...
		if (!Strings.isNullOrEmpty(kj.getBZ())) {
			...
			if ((InvoiceGenUtil.DKBZ_DKFP.equals(kj.getDKBZ()))
					&& ((kj.getBZ().indexOf("代开企业税号:") == -1) || (kj.getBZ()
							.indexOf("代开企业名称:") == -1)))
		...
		if (!Strings.isNullOrEmpty(kj.getQDXMMC())) {
		...
		if ((!Strings.isNullOrEmpty(kj.getMBDM()))
				&& (!checkLengthEquals(kj.getMBDM(), 4))) {
		...
		for (int i = 0; i < mxs.length; i++) {
			...
			if ((!Strings.isNullOrEmpty(mxs[i].getXMSL()))
			...
			if ((!Strings.isNullOrEmpty(mxs[i].getXMDJ()))
			...
			if (!Strings.isNullOrEmpty(mxs[i].getXMJE())) {
			...
			if ((!Strings.isNullOrEmpty(mxs[i].getXMSL()))
					&& (!Strings.isNullOrEmpty(mxs[i].getXMDJ()))
			...
			if (!Strings.isNullOrEmpty(mxs[i].getSL())) {
			...
			if (!Strings.isNullOrEmpty(mxs[i].getSE())) {
			...
			if ((!Strings.isNullOrEmpty(mxs[i].getSPBM()))
			...
			if ((!Strings.isNullOrEmpty(mxs[i].getZXBM()))
			...
			if ((!Strings.isNullOrEmpty(mxs[i].getYHZCBS()))
					&& (!InvoiceGenUtil.YHZCBS_FALSE.equals(mxs[i].getYHZCBS()))
					&& (!InvoiceGenUtil.YHZCBS_TRUE.equals(mxs[i].getYHZCBS()))) {
				throw new CustomException(1126, new StringBuilder().append("第")
						.append(i + 1).append("行优惠政策标识必须是0或1(")
						.append(mxs[i].getYHZCBS()).append(")").toString());
			}

			if ((!Strings.isNullOrEmpty(mxs[i].getLSLBS()))
					&& (!checkLengthEquals(mxs[i].getLSLBS(), 1))) {
				throw new CustomException(1127, new StringBuilder().append("第")
						.append(i + 1).append("行零税率标识长度有误(")
						.append(mxs[i].getLSLBS()).append(")").toString());
			}

			if (!Strings.isNullOrEmpty(mxs[i].getZZSTSGL())) {
				if (!InvoiceGenUtil.YHZCBS_TRUE.equals(mxs[i].getYHZCBS()))
					throw new CustomException(1138, new StringBuilder()
							.append("第").append(i + 1)
							.append("行使用优惠政策的商品需要填写正确的优惠政策标识(")
							.append(mxs[i].getYHZCBS()).append(",")
							.append(mxs[i].getZZSTSGL()).append(")").toString());
				if (checkLength(mxs[i].getZZSTSGL(), 50))
					throw new CustomException(1111, new StringBuilder()
							.append("第").append(i + 1)
							.append("行增值税特殊管理长度最大不能超过50(")
							.append(mxs[i].getZZSTSGL()).append(")").toString());
				String yhsl = checkZzstsgl_sl(mxs[i].getZZSTSGL(),
						mxs[i].getSL());
				if (!"success".equals(yhsl)) {
					throw new CustomException(1132, new StringBuilder()
							.append("第").append(i + 1).append("行使用(")
							.append(mxs[i].getZZSTSGL()).append(")优惠政策的商品只能享受")
							.append(yhsl).append("税率(").append(mxs[i].getSL())
							.append(")").toString());
				}
			} else if (InvoiceGenUtil.YHZCBS_TRUE.equals(mxs[i].getYHZCBS())) {
				throw new CustomException(1138, new StringBuilder().append("第")
						.append(i + 1).append("行使用优惠政策的商品必须填写增值税特殊管理信息(")
						.append(mxs[i].getYHZCBS()).append(")").toString());
			}

			if (!Strings.isNullOrEmpty(mxs[i].getKCE())) {
				try {
			...
			} else if ((i == 0) && (!Strings.isNullOrEmpty(kj.getBZ()))
					&& (kj.getBZ().indexOf("差额征税：") > -1)) {
		...
		if (ArithUtil.compareTo(ArithUtil.fdd(kj.getHJSE(), 2),
			...
			throw new CustomException(1091, new StringBuilder()
					...
					.append(")").toString());
		if (ArithUtil.compareTo(ArithUtil.fdd(kj.getHJSE(), 2),
				ArithUtil.fdd(hjse2, 2), new BigDecimal("1.27")) == 1)
			throw new CustomException(1152, new StringBuilder()
					.append("合计税额(")
					.append(kj.getHJSE())
					.append(")不等于商品行税额之和(")
					.append(hjse2)
					.append("=")
					.append(mxs.length < 15 ? hjseStr2.substring(0,
							hjseStr2.length() - 1) : new StringBuilder()
							.append(mxs.length).append("行税额之和").toString())
					.append(")").toString());
	}

	private static String checkZzstsgl_sl(String zzstsgl, String sl) {
	...
	private static boolean checkLength(String parmaterStr, int maxLength) {
		if (parmaterStr.length() > maxLength) {
			return true;
		}
		return false;
	}
	...
	public static boolean isZkh(String xmmc) {
		if ((xmmc.indexOf("折扣") == 0) || (xmmc.indexOf("折扣行数") == 0)) {
			return true;
		}
		return false;
	}

	public static String getSlStr(String spbm_bb, String zzstsgl)
			throws CustomException {
		if (!Strings.isNullOrEmpty(spbm_bb)) {
			if (Strings.isNullOrEmpty(zzstsgl)) {
				return "0%";
			}
